...
@media(max-width:768px){.article-wrap{padding:5rem 1rem 3rem}}
'''
...
<article class="article-wrap">
  ...
  <p>This guide focuses on practical decision-making. Instead of only listing moves, it explains how %s wants to play the round: what to fish for, what to punish, when to throw, when to stall, and how to use stage positioning to increase reward. That approach matters in DOA6 because the game is built around layered reads.</p>
  <p>Before committing to a main, test how the character feels in common situations: approaching from mid-range, checking aggressive opponents, recovering after a blocked starter, and closing rounds near the wall. If those situations feel comfortable, %s may be the right long-term main for your playstyle.</p>
  ...
  <ul>%s</ul>
  <p>These strengths define why %s can compete at a high level. The character is especially rewarding for players who like turning simple neutral wins into repeated pressure. In DOA6, small advantages snowball quickly: one good check forces respect, one blocked hit opens the throw game, and one correct read often opens the path to wall pressure or a round-ending sequence.</p>
  <h3>Main Weaknesses</h3>
  <ul>%s</ul>
  <p>Understanding weaknesses helps you build better practice habits. Instead of only grinding offense, test how the character handles common threats: fast rushdown, throw-heavy grapplers, evasive stance characters, and patient mid-range fighters. The more weaknesses you address early, the fewer free rounds you hand to opponents who already know the matchup.</p>
  ...
  <ul>%s</ul>
  <p>This foundation works because it mirrors the way DOA6 matches actually unfold. You begin by testing the opponent with safe pressure, then you expand once you see how they defend. If they press too much, you check them. If they block too much, you throw. If they try to escape with predictable movement, you call it out with a mid that catches their habit.</p>
  <p>For beginners, the most important skill is hit confirmation. Build the habit of confirming whether your first hit connected or was blocked, then choose the right follow-up accordingly. This single habit will improve your win rate more than memorizing long combo routes.</p>
  ...
  <p>Another advanced layer is meter management. The Break Gauge gives you access to powerful options, but only if you spend it at the right time. Good %s players think one round ahead.</p>
...
''' % (
        ...
        li(c['strengths']), esc(c['name']), li(c['weaknesses']), esc(c['name']), li(c['beginner']), esc(c['name']), li(c['advanced']), esc(c['name']), esc(c['name']), esc(c['matchup_tip']), esc(c['strat']), esc(c['name']), c['tier'], esc(c['name']), esc(c['style']), esc(c['name']), esc(c['name']),
...
print('generated', len(chars), 'guide pages')
